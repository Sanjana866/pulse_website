...
const Register = () => {
  ...
  const handleRegister=async (e)=>{
    ...
    try {
      const url="https://pulse-website.onrender.com/api/auth/signup";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F4FAF3] px-4">
      <div className=" bg-cover 
          bg-center h-[30rem] md:h-[40rem] bg-[#F4FAF3]" style={{ backgroundImage: `url(${book})` }}/>
      ...
      className={`bg-white p-8 rounded-lg shadow-md w-full max-w-md transition-opacity duration-1000 ${isVisible? 'animate-fade-in-up':'opacity-100'}`}>
        <h2 className="text-2xl font-bold mb-6 text-center text-[#612D2D]">Register</h2>
        <form className="space-y-5" onSubmit={handleRegister}>
          ...
          <Link to="/login" className='text-sm font-bold text-green-700/75 underline'>Login in now</Link>
          </div>
        </form>
      </div>
      <ToastContainer/>
